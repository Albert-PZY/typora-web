...
| `style.css` | Website and harness CSS; reusable editor CSS lives under `src/styles/`. |
...
4. Run again; expect green. Same `events` array shows up in the harness as a preset automatically (via `collectCases`) — open `pnpm dev` and eyeball step-by-step.
